...
- There is a search bar located on the navigation bar, that is always accessible to users. Users can quickly search for the product they are looking for, using this functionality.
...
- The user can use the remove link to remove items from their shopping bag.
...
- There is a section of the profile page that contains the user's previous orders, called order history.
- This is displayed in a column to the right, on larger screens, and at the top on mobile devices.
...
- This is achieved through the implementation of Bootstrap toasts. I have set up toasts for warnings, errors, information messages, and success messages.
...
- This website provides a 'Contact Us' page, which can be accessed directly from the navigation bar, this is always visible to the user.
...
- These update and delete links are provided in the product detail page page, directly below the product image.
...
- These update and delete links are provided in the product detail page directly below each product feature.
...
| **_Search Bar Empty Error Message_** | Submit a search using the search bar with an empty search field |  An error message appears at the top right hand corner, telling the user they did not enter any search criteria | **Pass** - the error message is displayed as expected. |   |
...
| **_Sign In Form - invalid_** | Navigate to sign in page, and enter an incorrect username or password |  User is alerted of the error, and directed to the field that requires fixing |  **Pass** - user cannot log in if form is invalid, and is prompted to fix the errors on the specified fields |   |
...
| **_Thank you Page button takes user back to All Products Page_** | Complete and submit the contact form, this will direct the user to the thank you page, click the button 'Check Out Our Styled Services'  |  Button works, and directs the user to the All Products Page | **Pass** - This test was a success, and button works as expected.|   |
...
| **_Shopping Bag Page - Update link works_** | Add items to the bag, and navigate to the shopping bag page, and increase or decrease the quantity for the item, click the update link |  This update link updates the quantity of the item to the new amount |  **Pass** - this test was successful, and the update link correctly adjust the quantity of the item in the shopping bag as expected. |   |
| **_Shopping Bag Page - Remove link works_** | Add items to the bag, and navigate to the shopping bag page, and click the remove link  |  Item is removed from the user's shopping bag |  **Pass** - this test was a success, and the item is successfully removed from the user's bag and works as expected. |   |
...
| **_Checkout form will not be submittted unless valid_** | Complete the checkout order form with incorrect details, such an invalid email field |  Form will not be submitted, and any errors are highlighted, and user is directed to these fields, to correct any errors before submitting. |  **Pass** - the checkout order form will not be submmitted unless it is valid. Test was a success. |   |
| **_Checkout form card errors are displayed to the user_** | Fill in incorrect card details, in the payment section of the checkout page |  User is alerted to payment errors, and an error message is displayed.|  **Pass** - card errors on checkout page are highlighted, and displayed to the user as expected. |   | 
...
|**_Add Product Page - form must be valid to submit successfully_** | Log in as Admin user, and navigate to the Product Management link in the 'My Account' dropdown menu, and try to submit an empty form, and then submit a form with errors  |  The form will not be submitted, and errors and fields to be completed will be highlighted for the user |  **Pass** - this test was a success, and form cannot be submitted unless it is valid, worked as expected. |   |
...
| **_Edit Product Page - form must be valid to submit successfully_** | Log in as Admin user, and navigate to the product detail page, and click the edit link under the product image, and submit an empty form, and then submit a form with errors  |  The form will not be submitted, and errors and fields to be completed will be highlighted for the user |  **Pass** - this test was a success, and form cannot be submitted unless it is valid, worked as expected. |   |
...
| **_Add Product Feature Page - form must be valid to submit successfully_** |  Log in as Admin user, and navigate to the Product Feature Management link in the 'My Account' dropdown menu, and submit an empty form, and then submit a form with errors  |  The form will not be submitted, and errors and fields to be completed will be highlighted for the user |  **Pass** - this test was a success, and form cannot be submitted unless it is valid, worked as expected. |   |
...
| **_Edit Product Feature Page - form must be valid to submit successfully_** | Log in as Admin user, and navigate to the product detail page, and click the edit link under the product feature, and submit an empty form, and then submit a form with errors  |  The form will not be submitted, and errors and fields to be completed will be highlighted for the user |  **Pass** - this test was a success, and form cannot be submitted unless it is valid, worked as expected. |   |
...
| **_Add Image Page - form must be valid to submit successfully_** | Log in as Admin user, and navigate to the Look Book Management link in the 'My Account' dropdown menu, and submit an empty form, and then submit a form with errors  |  The form will not be submitted, and errors and fields to be completed will be highlighted for the user |  **Pass** - this test was a success, and form cannot be submitted unless it is valid, worked as expected. |   |
...
| **_Add Image Page - if form is valid the Add Image button works_** | Complete the add image form, and click the add image button  |  Add Image form is submitted (if valid) |  **Pass** - Add Image button works as expected. |   |
...
| **_Edit Image Page - form must be valid to submit successfully_** | Log in as Admin user, and navigate to the gallery page, and click the edit link under the image, and submit an empty form, and then submit a form with errors  |  The form will not be submitted, and errors and fields to be completed will be highlighted for the user |  **Pass** - this test was a success, and form cannot be submitted unless it is valid, worked as expected. |   |
...
#### **Automated Testing**
- I had hoped to implement some automated tests on my models and views using the built in Django testing framework, however, due to time constraints I was unabale to implement these tests before submission for this project. I have however, been testing all website functionality, database and email configuration using TDD(Test Driven Developement) throughout the development of the project.

### **Issues Encountered**

**_Webhooks failing in develeopment_**

**Issue**
- When I set up the webhook handler functionality and logged into my Stripe account, I noticed that all webhooks including test webhooks had failed.

**Fix**
- After doing some research, I found a solution on Slack, I changed the exposed port to public instead of private and this action solved the issue.
- All webhooks were successful after this change. The issue was resolved.
...
- I then set a variable `HEROKU_ON` to True in Heroku config vars, and used the if statement `if HEROKU_ON in os.environ`, and this resolved the issue. I was then able to connect to both databases, with the DATABASE_URL variable I had set in Heroku.
...
**_Email Configuration with Gmail_**

**Issue**
- Emails in production returning 500 internal error page, and not sending email to Gmail account on form submit.

**Fix**
- I initially had set the `EMAIL HOST` and `EMAIL PORT` variables in Heroku, I changed these and put the email host and port directly in the seetings.py file, moved the code to the bottom of the file, and changed the placement of the variables, this resolved the issue.
- This resolved the issue, and emails are now being sent successfully to the company Gmail account on contact form submit and order confirmation.


**_Migrating data to PostgreSQL database_**

**Issue**
- When I was trying to migrate my data to the production database, it was throwing an error, that the Heroku database did not exist.
...
**Fix**
- After doing some research I found a solution to this issue, I set the body to `display: flex` and `flex-direction: column`, and set the footer to have `margin:auto`, `bottom:0`.
...
**Issue**
- Issue with triggering the Bootstrap toasts, they were not showing up on the screen after user actions.

**Fix**
- Using the Bootstrap 5 documentation, I was able to resolve this issue using the JS code snippet below.
- Code snippet used:
